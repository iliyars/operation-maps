...
public static class DatabaseSeeder
{
  public static async Task SeedAsync(OperationMapsDbContext db)
  {
    ...
    var parameters = new List<FormParameter>
        {
            new() { Form = form4, Section = secReliability, RowNumber = 1,  Order = 1,
                    Name = "Показатель ресурса",                                  Unit = "ч" },
            new() { Form = form4, Section = secReliability, RowNumber = 2,  Order = 2,
                    Name = "Показатель срока службы",                             Unit = "лет" },
            new() { Form = form4, Section = secReliability, RowNumber = 3,  Order = 3,
                    Name = "Показатель сохраняемости",                            Unit = "лет" },
            new() { Form = form4, Section = secOperation,   RowNumber = 4,  Order = 1,
                    Name = "Акустический шум: диапазон частот",                   Unit = "Гц" },
            new() { Form = form4, Section = secOperation,   RowNumber = 5,  Order = 2,
                    Name = "Акустический шум: уровень звукового давления",        Unit = "дБ" },
            new() { Form = form4, Section = secOperation,   RowNumber = 6,  Order = 3,
                    Name = "Линейное ускорение",                                  Unit = "М.С.Е-2.(G)" },
            new() { Form = form4, Section = secOperation,   RowNumber = 7,  Order = 4,
                    Name = "Давление окружающей среды: пониженное",               Unit = "мм рт. ст." },
            new() { Form = form4, Section = secOperation,   RowNumber = 8,  Order = 5,
                    Name = "Давление окружающей среды: повышенное",               Unit = "атм" },
            new() { Form = form4, Section = secOperation,   RowNumber = 9,  Order = 6,
                    Name = "Предельная (рабочая) температура: пониженная",        Unit = "°С" },
            new() { Form = form4, Section = secOperation,   RowNumber = 10, Order = 7,
                    Name = "Предельная (рабочая) температура: повышенная",        Unit = "°С" },
            new() { Form = form4, Section = secOperation,   RowNumber = 11, Order = 8,
                    Name = "Относительная влажность",                             Unit = "%" },
            new() { Form = form4, Section = secOperation,   RowNumber = 12, Order = 9,
                    Name = "Относительная влажность: температура",                Unit = "°С" },
            new() { Form = form4, Section = secOperation,   RowNumber = 13, Order = 10,
                    Name = "Роса, иней",                                          Unit = null },
            new() { Form = form4, Section = secOperation,   RowNumber = 14, Order = 11,
                    Name = "Стойкость к воздействию специальных факторов, 7К",    Unit = null },
        };
    ...
    db.ComponentTypes.AddRange(typeResistor, typeCapacitor, typeMicrocircuit,
                               typeGenerator, typeConnector);
    db.TypeForms.AddRange(
        new TypeForm { ComponentType = typeResistor, Form = form4 },
        new TypeForm { ComponentType = typeCapacitor, Form = form4 },
        new TypeForm { ComponentType = typeMicrocircuit, Form = form4 },
        new TypeForm { ComponentType = typeGenerator, Form = form4 },
        new TypeForm { ComponentType = typeConnector, Form = form4 }
    );

    // ── 3. Справочник примечаний ─────────────────────────────────────────
    ...
    // Р1-8МП — без примечаний
    var famR18MP = new Family { Name = "Р1-8МП", ComponentType = typeResistor };
    ...
    // Первый SaveChanges — все сущности получают Id
    await db.SaveChangesAsync();

    // ── 5. FamilyNote — только после получения Id ★ ──────────────────────
    db.FamilyNotes.AddRange(
        new FamilyNote
        {
          FamilyId = famK1079.Id,
          FormParameterId = P(1).Id,
          NoteId = noteOblRezhim.Id,
        },
        new FamilyNote
        {
          FamilyId = famK5367.Id,
          FormParameterId = P(1).Id,
          NoteId = noteOblRezhim.Id,
        }
    );
